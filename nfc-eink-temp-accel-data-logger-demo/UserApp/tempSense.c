 ...
 *
 */
resultData tempSense(uint8_t* dataPack){
//	uint16_t IntDegF;
	...
	IntDegC = ((temp*93)>>4)-2860; 			//2860 is the offset for some board , 2650 for other board 2700
//	data.y=Y_offset-(IntDegC>>1);//
	dataPack[1] = Y_offset-(IntDegC>>1);
	//data.x = senseState + X_start;
	// data.x = senseState + 3;
	dataPack[0] = senseState + 3;
...
//	if(!((Y_start<data.y)&(data.y<Y_end))){
	...
	if(!((Y_start<dataPack[1])&(dataPack[1]<Y_end))){
		dataPack[1] = Y_start;
	}
